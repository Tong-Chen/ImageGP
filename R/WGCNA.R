...
#' @param modTraitCorP A dataframe generated by \code{\link{WGCNA_moduleTraitPlot}}
#'
#' ```
#' Module	Trait	PersonCorrelationValue	Pvalue
#' MEblack	Glucose	0.204755937	0.021454045
#' MEblack	Glucose_Insulin	-0.124720673	0.172877729
#' MEblack	Insulin_ug_l	0.044518191	0.62777743
#' ```
#'
#' @param pvalue_filter_threshold The pvalue threshold for filtering module-trait pairs for generating the scatter plot (default 0.05).
#' @inheritParams WGCNA_saveModuleAndMe
#'
#'
#' @return NULL
...
WGCNA_GeneModuleTraitCoorelation <-
  function(datExpr,
    ...
    # for (module in modNames) {
      ...
      #   if (!sp.is.null(modTraitCorP)) {
      #     module_pheno_p = modTraitCorP[modTraitCorP$Module==paste0("ME", module) & modTraitCorP$Trait==pheno, "Pvalue"]
      #     if (module_pheno_p > pvalue_filter_threshold) {
      #       next
      #     }
      #   }
        ...
        pheno_column = match(pheno, phenoName)
        # 获取模块内的基因
